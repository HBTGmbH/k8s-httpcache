...
| terminationGracePeriodSeconds | int | `90` | Termination grace period in seconds. Must cover the WORST-CASE sequential shutdown budget: drain.delay + drain.timeout + broadcast drain (30s) + broadcast shutdown (5s) + varnishncsa stop (5s, when access logging is enabled) + vcl shutdown-timeout (30s) + metrics drain (5s), plus slack. The controller logs a warning at startup when its configured budget exceeds this value (the kubelet would SIGKILL varnishd mid-drain). |
